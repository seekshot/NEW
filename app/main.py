...
@app.post("/demo/seed", summary="Seed mock data for the demo UI")
def seed_demo_data(db: Session = Depends(get_db)):
    """
    Seeds a mock account, demo rules, loads sample transactions, and applies rules.
    This is safe for local/demo use only and does not touch real banking data.
    """
    ...
    account_name = "TD Chequing (Demo)"
    account = db.query(Account).filter(Account.name == account_name).first()
    accounts_created = 0
    if not account:
        account = crud.create_account(db, name=account_name, provider="MOCK", institution="TD")
        accounts_created = 1

    # Seed demo rules
    demo_rules = [
        {"pattern": "UBER", "field": "merchant", "category": "Transport", "bucket": "Transport", "priority": 100},
        {"pattern": "WHOLE FOODS", "field": "merchant", "category": "Food", "bucket": "Food", "priority": 90},
        {"pattern": "NETFLIX", "field": "merchant", "category": "Entertainment", "bucket": "Entertainment", "priority": 80},
    ]
    rules_created = 0
    ...
    # Load and insert demo transactions
    with data_path.open() as f:
        raw_transactions = json.load(f)
    ...
    transactions_inserted = crud.bulk_insert_transactions(db, account, normalized)

    # Apply rules to demo data
    processed = crud.apply_rules(db, account_id=account.id)
...
def demo_page(request: Request, db: Session = Depends(get_db)):
    ...
    total_spend = float(total_value * -1 if total_value < 0 else total_value)
    ...
    bucket_summaries = [
        {
            "bucket": row.bucket or "Uncategorized",
            "tx_count": row.count,
            "total": float((row.total or 0) * -1 if (row.total or 0) < 0 else (row.total or 0)),
        }
